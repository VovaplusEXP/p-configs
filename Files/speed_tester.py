...
# --- V2Ray/Xray Config Generation (Logic is sound, no changes needed here) ---
def create_v2ray_config(proxy_line, local_socks_port, worker_id):
    ...
    try:
        if protocol == "vless":
            ...
            outbound_config = {"protocol": "vless", "settings": {"vnext": [{"address": parsed_url.hostname, "port": parsed_url.port, "users": [{"id": parsed_url.username, "flow": qs.get("flow", [""])[0]}]}]}, "streamSettings": {"network": qs.get("type", ["tcp"])[0], "security": qs.get("security", ["none"])[0], "tlsSettings": {"serverName": qs.get("sni", [parsed_url.hostname])[0]} if qs.get("security", ["none"])[0] == "tls" else {}, "wsSettings": {"path": qs.get("path", ["/"])[0]} if qs.get("type", ["tcp"])[0] == "ws" else {}}}
        elif protocol == "vmess":
            b64_part = proxy_line[len("vmess://"):]
            b64_part += '=' * (-len(b64_part) % 4)
            vmess_data = json.loads(base64.b64decode(b64_part).decode('utf-8'))
            outbound_config = {"protocol": "vmess", "settings": {"vnext": [{"address": vmess_data.get("add"), "port": int(vmess_data.get("port")), "users": [{"id": vmess_data.get("id"), "alterId": int(vmess_data.get("aid"))}]}]}, "streamSettings": {"network": vmess_data.get("net"), "security": "tls" if vmess_data.get("tls") == "tls" else "none", "tlsSettings": {"serverName": vmess_data.get("host", vmess_data.get("add"))} if vmess_data.get("tls") == "tls" else {}, "wsSettings": {"path": vmess_data.get("path", "/"), "headers": {"Host": vmess_data.get("host", vmess_data.get("add"))}} if vmess_data.get("net") == "ws" else {}}}
        elif protocol == "trojan":
            parsed_url = urlparse(proxy_line)
            qs = parse_qs(parsed_url.query)
            outbound_config = {"protocol": "trojan", "settings": {"servers": [{"address": parsed_url.hostname, "port": parsed_url.port, "password": parsed_url.username}]}, "streamSettings": {"network": qs.get("type", ["tcp"])[0], "security": qs.get("security", ["none"])[0], "tlsSettings": {"serverName": qs.get("sni", [parsed_url.hostname])[0]} if qs.get("security", ["none"])[0] == "tls" else {}, "wsSettings": {"path": qs.get("path", ["/"])[0]} if qs.get("type", ["tcp"])[0] == "ws" else {}}}
        elif protocol == "ss":
            parsed_url = urlparse(proxy_line)
...
def test_proxy(proxy_line, worker_id):
    ...
    # FIX 1: Handle malformed configs and return 0 for speed to prevent TypeError
    if not config_file:
        return 0, proxy_line, "Malformed"

    # FIX 3: Remove DEVNULL to see Xray's output in logs
    process = subprocess.Popen([os.path.abspath(V2RAY_EXECUTABLE_PATH), "run", "-c", config_file])

    # FIX 2: Replace time.sleep with a robust readiness check
    is_ready = False
    ...
    if is_ready:
        try:
            ...
            status = f"Error ({type(e).__name__})" # More specific error status
        except Exception:
